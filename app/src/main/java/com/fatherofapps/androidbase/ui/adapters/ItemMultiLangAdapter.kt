...
import com.fatherofapps.androidbase.data.models.LanguageTrans
import com.fatherofapps.androidbase.databinding.ItemMultiLangBinding
import com.fatherofapps.androidbase.ui.multi_lang.MultiLangFragment
...
class ItemMultiLangAdapter(
    ...
    companion object {
        val dummyData = listOf(
            ...
            LanguageTrans(name = "Chinese (Simplified)", code = "zh-CN", avatar = R.drawable.color_china),
            LanguageTrans(name = "Chinese (Traditional)", code = "zh-TW", avatar = R.drawable.color_china),
            LanguageTrans(name = "Czech", code = "cs", avatar = R.drawable.color_czech_republic),
            LanguageTrans(name = "English", code = "en", avatar = R.drawable.color_unitedkingdom),
            ...
            LanguageTrans(name = "Vietnamese", code = "vi", avatar = R.drawable.color_vn),
        )
    }

    class ViewHolder(val binding: ItemMultiLangBinding, private val context: Context) : RecyclerView.ViewHolder(binding.root) {

        fun bind(item: LanguageTrans, isSelected: Boolean) {
            binding.apply {
                if (isSelected) {
                    itemLangCardView.setBackgroundColor(Color.LTGRAY)
                    itemLangImgCheck.setImageResource(R.drawable.checked)
                } else {
                    itemLangCardView.setBackgroundColor(Color.WHITE)
                    itemLangImgCheck.setImageResource(R.drawable.not_check)
                }
